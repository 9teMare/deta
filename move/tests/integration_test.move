...
module datax::integration_test {
    ...
    fun test_full_workflow() {
        ...
        // 2. User1 submits data
        data_registry::submit_data(&user1, b"data_hash_1", b"metadata_1");
        data_registry::submit_data(&user1, b"data_hash_2", b"metadata_2");

        // 3. Verify data is stored
        assert!(data_registry::get_dataset_count(USER1) == 2, 1);
        assert!(UserVault::get_dataset_count(USER1) == 2, 2);

        // 4. User1 grants access to buyer
        let current_time = timestamp::now_seconds();
        let expires_at = current_time + 86400; // 24 hours
        AccessControl::grant_access(&user1, 0, DATA_BUYER, expires_at);

        // 5. Verify access
        assert!(
            AccessControl::has_access(USER1, 0, DATA_BUYER) == true,
            3
        );
        assert!(
            AccessControl::has_access(USER1, 1, DATA_BUYER) == false,
            4
        );
        ...
        AccessControl::revoke_access(&user1, 0, DATA_BUYER);
        assert!(
            AccessControl::has_access(USER1, 0, DATA_BUYER) == false,
            5
        );
        ...
        assert!(UserVault::get_dataset_count(USER1) == 1, 7); // Removed from vault
    }
    ...
    fun test_multiple_users_data_network() {
        ...
        data_registry::submit_data(&user1, b"user1_data", b"user1_meta");
        data_registry::submit_data(&user2, b"user2_data", b"user2_meta");
        ...
        assert!(UserVault::get_dataset_count(USER1) == 1, 10);
        assert!(UserVault::get_dataset_count(USER2) == 1, 11);

        // Each user controls their own access
        let current_time = timestamp::now_seconds();
        AccessControl::grant_access(&user1, 0, DATA_BUYER, current_time + 3600);
        AccessControl::grant_access(&user2, 0, DATA_BUYER, current_time + 3600);
        ...
        assert!(
            AccessControl::has_access(USER2, 0, DATA_BUYER) == true,
            13
        );
    ...
    fun test_user_data_sovereignty() {
        ...
        // User1 submits data
        data_registry::submit_data(&user1, b"sensitive_data", b"private_meta");

        // User2 cannot access without permission
        assert!(
            AccessControl::has_access(USER1, 0, USER2) == false,
            16
        );
        ...
        assert!(UserVault::has_dataset(USER1, 0) == false, 18);
    }
}
